    ...
    def run(self):
        ...
        while not user_exit:
            ...
            try:
                ...
                pass                 
            print(f"Task 2 id = {self.id} run={self.loop_ctr}")
        try:
            del seed_head
        except NameError:
            pass
...
class Sequencer():
    ...
    def addTaskToDict(self, task_id, task_data):
        ...
        #For all tasks. 
        try:
            self.task_ctl_dict[task_id] = task_data
        except:
            pass
        #setup pause control
        self.setTaskCtlDict(task_id, False, 0)
            
    def setTaskCtlDict(self, task_id, pause_state, delay):
    	  #Sets up the sync control for a task
        try:
            self.task_ctl_dict[task_id]['pause'] = pause_state
            self.task_ctl_dict[task_id]['delay'] = delay
        except:
            self.task_ctl_dict[task_id]={'pause': pause_state, 'delay':delay}         
    ...
    def taskControl(self, task_id):
        ...
        global c_warped
        t = self.task_ctl_dict[task_id]
        #print(f"task: {task_id} warped: {c_warped}")
        ctr=0
        while c_warped == True:
            if t['pause'] == True:
                time.sleep(t['delay'])
