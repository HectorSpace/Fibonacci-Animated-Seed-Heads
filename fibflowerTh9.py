...
flight_period_const = 0.2/slug_factor
lull_period = 1
lull_increment = 0.5 * slug_factor ** 2
star_density = 0.01 / slug_factor ** 2
    ...
    def draw(self, gwin):
        ...
        if self.starFieldControl == 1:
            for star in self.stars:
                star.draw(gwin)
                if user_exit: break
            self.starFieldControl = 2
        elif self.starFieldControl > 1:
            #allow a few frames before delete
            self.starFieldControl += 1
...
class StarFieldTask(threading.Thread):
    ...
    def run(self):
        ...
        while not user_exit:
            start = time.time()
            self.star_field.starFieldControl = 0
            ...
            for star in self.star_field.stars:
                ...
                star.x += (star.x - centerX) * star.size * line_length
                star.y += (star.y - centerY) * star.size * line_length
...
def frameTimer(start, end, frames):
    global lull_period
    drawTime= end - start
    lapseTime = frameTime*frames
    if drawTime < frameTime:
        if drawTime <= (frameTime/2):
            lull_period += lull_increment
        elif lull_period > (1 + lull_increment) :
            lull_period -= lull_increment
        time.sleep(frameTime-drawTime)
    else:
        lull_period = 1
        time.sleep(0.001)
